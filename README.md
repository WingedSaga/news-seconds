...
| `RESEND_API_KEY`            | Ключ [Resend](https://resend.com); включает отправку по HTTPS |
...
Способ отправки выбирается сам: есть `RESEND_API_KEY` — письма идут через
HTTPS-API Resend, иначе через SMTP, а если не задано ничего — подтверждение
почты выключается. Resend предпочтительнее: хостинги часто фильтруют
исходящий SMTP, и письма тогда молча не уходят.
