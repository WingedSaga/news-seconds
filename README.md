...
- Страница статьи: галерея изображений, счётчик просмотров, проигрыватель
  вложенного аудио или видео, комментарии
...
- Отправка своей новости: заголовок, текст, категория, до пяти изображений
  и вложение MP3 или MP4 до 100 МБ: то, что не влезает в хранилище,
  сервер пережимает сам через ffmpeg
- Отслеживание статуса своих материалов: на модерации / одобрено / отклонено
- Комментарии к статьям
- Закладки
- Профиль: загрузка аватара и смена имени. Без своей картинки показываются
  инициалы на цветном круге, цвет выводится из имени — у человека он всегда один
- Обращения в службу поддержки с перепиской прямо на странице

**Админ-панель** (`/admin`, только для роли `admin`)

- **Обзор** — восемь показателей, график публикаций за две недели, распределение
  по разделам и статусам, самые читаемые материалы, очередь проверки,
  последние комментарии
- **Модерация** — предпросмотр, одобрение и отклонение
- **Все статьи** — поиск, фильтр по статусу, редактирование, удаление и
  массовые действия над выбранными материалами
- **Комментарии** — просмотр всех комментариев сайта с поиском и удалением
- **Пользователи** — поиск, роли, блокировка, удаление аккаунта
- **Поддержка** — обращения читателей, ответы и статусы
- **Журнал** — кто из администраторов что менял
- **Управление** — переключатели сайта, название и подзаголовок,
  назначение администратора по адресу почты, выгрузка данных в CSV

Переключатели раздела «Управление»: регистрация, подтверждение почты,
комментарии, режим обслуживания и публикация без модерации. Применяются сразу,
без передеплоя. В режиме обслуживания сайт отвечает заглушкой всем, кроме
администраторов, — вход и панель продолжают работать.
...
server/                    Express API
  ...
  services/mailer.js       письма подтверждения (Brevo, Resend или SMTP)
  services/settings.js     настройки сайта из базы с кэшем
  services/audit.js        журнал действий администраторов
...
| `SUPABASE_MEDIA_BUCKET`     | Бакет для аудио и видео (`article-media`)                    |
...
| Метод  | Маршрут       | Доступ | Описание                                       |
| ------ | ------------- | ------ | ---------------------------------------------- |
| `POST` | `/api/upload`       | Токен  | Картинка: `multipart/form-data`, поле `image`, до 5 МБ  |
| `POST` | `/api/upload/media` | Токен  | MP3 или MP4: поле `media`, до 100 МБ                    |
