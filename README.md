...
- После регистрации на указанный адрес уходит письмо со ссылкой, вход до
  подтверждения закрыт
- Ссылка живёт 24 часа, повторную отправку можно запросить со страницы входа
- В базе хранится не сам токен, а его SHA-256 хеш
- Отправка через Resend (HTTPS) или обычный SMTP — что настроено
- Если не настроено ничего, проверка почты выключается целиком
  и аккаунты создаются сразу подтверждёнными — удобно для локальной разработки
...
server/                    Express API
  ...
  db/schema.sql            схема БД, права, бакет и функция инкремента просмотров
  db/migrations/           миграции для уже развёрнутых баз
  services/mailer.js       письма подтверждения через SMTP
...
| `CLIENT_URL`                | Адрес фронтенда, из него собирается ссылка в письме          |
| `BREVO_API_KEY`             | Ключ [Brevo](https://brevo.com); отправка по HTTPS            |
| `RESEND_API_KEY`            | Ключ [Resend](https://resend.com); отправка по HTTPS          |
| `SMTP_HOST`, `SMTP_PORT`    | SMTP-сервер, по умолчанию `smtp.gmail.com:465`               |
| `SMTP_USER`, `SMTP_PASS`    | Учётные данные SMTP; для Gmail — пароль приложения           |
| `MAIL_FROM`                 | Отправитель писем                                            |

Способ отправки выбирается сам, в порядке `BREVO_API_KEY` → `RESEND_API_KEY`
→ SMTP; если не задано ничего, подтверждение почты выключается. Оба API
работают поверх HTTPS, и это важно: хостинги часто фильтруют исходящий SMTP,
и письма тогда молча не уходят.

Разница между сервисами:

- **Brevo** — подтверждается один адрес отправителя в разделе Senders,
  свой домен не нужен, письма уходят на любые адреса, 300 писем в сутки.
  `MAIL_FROM` обязателен и должен совпадать с подтверждённым адресом.
- **Resend** — без подтверждённого домена шлёт только на адрес владельца
  аккаунта. Отправитель по умолчанию `onboarding@resend.dev`; публичные
  почтовые домены в `MAIL_FROM` игнорируются, так как Resend их отвергает.
...
| `POST` | `/api/auth/verify-email`         | —      | Подтверждение почты по токену из письма  |
